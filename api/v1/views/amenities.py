...
'''Defines allowed methods for the amenities endpoint.'''
...
def handle_amenities(amenity_id=None):
    '''The method handler dictionary for amenities.
    '''
    handlers = {
        'GET': get_amenities,
...
def get_amenities(amenity_id=None):
    '''Gets amenity obj with given id or all amenities.
    '''
...
def remove_amenity(amenity_id=None):
    '''Removes amenity object with given id.
    '''
    all_amenities = storage.all(Amenity).values()
    res = list(filter(lambda x: x.id == amenity_id, all_amenities))
    if res:
...
def update_amenity(amenity_id=None):
    '''Updates amenity object with given id.
    '''
