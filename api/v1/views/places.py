...
def handle_places(city_id=None, place_id=None):
    '''Method handler dictionary for places endpoint.
    '''
    handlers = {
        'GET': get_places,
        'DELETE': remove_place,
...
def get_places(city_id=None, place_id=None):
    '''
    Gets place obj with given id or all places in
    the city with given id.
    '''
    if city_id:
...
def remove_place(city_id=None, place_id=None):
    '''Removes place object with given id.
    '''
    if place_id:
        place = storage.get(Place, place_id)
        if place:
...
def update_place(city_id=None, place_id=None):
    '''Updates place object with given id.
    '''
...
def find_places():
    '''Finds places using lists of State, City, or Amenity ids.
    '''
    data = request.get_json()
    if type(data) is not dict:
        raise BadRequest(description='Not a JSON')
