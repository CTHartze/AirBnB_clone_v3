#!/usr/bin/python3
'''Contains API places_amenities view.'''
...
def get_place_amenities(place_id=None, amenity_id=None):
    '''Gets amenities of a place with given id.
    '''
    if place_id:
...
def remove_place_amenity(place_id=None, amenity_id=None):
    '''Removes amenity objects with given id from a place with given id.
    '''
...
def add_place_amenity(place_id=None, amenity_id=None):
    '''Adds/Links amenity object with given id to a place with given id.
    '''
    if place_id and amenity_id:
        place = storage.get(Place, place_id)
        if not place:
