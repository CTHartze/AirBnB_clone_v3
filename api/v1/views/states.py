...
'''Defines allowed methods for the states endpoint.'''
...
def handle_states(state_id=None):
    '''The method handler dictionary for states.
    '''
    handlers = {
        'GET': get_states,
        'DELETE': remove_state,
...
def get_states(state_id=None):
    '''Gets state object with given id or all states.
    '''
...
def remove_state(state_id=None):
    '''Removes state object with given id.
    '''
    all_states = storage.all(State).values()
    res = list(filter(lambda x: x.id == state_id, all_states))
    if res:
...
def update_state(state_id=None):
    '''Updates state object with given id.
    '''
